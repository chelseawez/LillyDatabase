...
from db_beers.models import Beer, Bar, Drinker, Frequents, Likes
...
def logout(request):
    auth_logout(request)
    return HttpResponseRedirect(reverse('db_beers.views.all_drinkers'))

def all_drinkers(request):
    return render_to_response('db_beers/all-drinkers.html',
...
def drinker(request, drinker_name):
    ...
    return render_to_response('db_beers/drinker.html',
        { 'drinker' : drinker, 
...
@login_required(login_url=reverse_lazy('db_beers.views.login'))
def edit_drinker(request, drinker_name):
    ...
    return render_to_response('db_beers/edit-drinker.html',
        { 'drinker' : drinker,
...
def edit_drinker_submit(request, drinker_name):
    ...
    # the back button.
    return HttpResponseRedirect(reverse('db_beers.views.drinker', args=(drinker.name,)))
